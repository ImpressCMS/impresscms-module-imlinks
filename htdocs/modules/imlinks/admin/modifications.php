...
switch ( strtolower( $op ) ) {
	case 'listmodreqshow':
		...
		$sform = new icms_form_Theme( _AM_IMLINKS_MOD_ORIGINAL, 'storyform', 'links.php' );
	...
	case 'changemodreq':
		...
		redirect_header( 'links.php', 1, _AM_IMLINKS_MOD_REQUPDATED );
		break;

	case 'ignoremodreq':
		$sql = sprintf( 'DELETE FROM ' . icms::$xoopsDB -> prefix( 'imlinks_mod' ) . ' WHERE requestid=' . $requestid );
		icms::$xoopsDB -> query( $sql );
		redirect_header( 'links.php', 1, _AM_IMLINKS_MOD_REQDELETED );
		break;
