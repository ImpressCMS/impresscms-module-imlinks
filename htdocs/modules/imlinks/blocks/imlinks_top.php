...
function b_imlinks_top_edit( $options ) {
	...
	$form .= '<tr><td><b>' . _MB_IMLINKS_DATEFORMAT . ':</b></td>';
...
function b_imlinks_recent_show( $options ) {
	...
	while ( $myrow = icms::$xoopsDB -> fetchArray( $result ) ) {
				...
				if ( !empty( $myrow['screenshot'] ) && file_exists( ICMS_ROOT_PATH . '/' . $imlModuleConfig['screenshots'] . '/' . trim( $myrow['screenshot'] ) ) ) {
					if ( isset( $imlModuleConfig['usethumbs'] ) && $imlModuleConfig['usethumbs'] == 1 ) {
						...
						if ( $imlModuleConfig['uselyte'] == 1 ) {
							...
							$linkload['autothumbsrc'] = '<a title="' . $myrow['title'] . '" href="' . ICMS_URL . '/' . $imlModuleConfig['screenshots'] . '/' . trim( $myrow['screenshot'] ) . '" target="_blank"><img src="' . ICMS_URL . '/' . $imlModuleConfig['screenshots'] . '/' . $_image . '" alt="" title="' . _MB_IMLINKS_SCRSHOTENLARGE . '"  /></a>';
						}
					} else {
		...
		$xoopsTpl -> assign( 'dirname', basename( dirname( dirname( __FILE__ ) ) ) );
		$xoopsTpl -> assign( 'uselyte', $imlModuleConfig['uselyte'] );
		$block['links'][] = $linkload;
	}
	unset( $linkload );
	return $block;
...
function b_imlinks_recent_edit( $options ) {
	...
	$form .= '<tr><td><b>' . _MB_IMLINKS_DATEFORMAT . ':</b></td>';
	$form .= '<td><input type="text" name="options[]" value="' . $options[1] . '" />&nbsp;' . _MB_IMLINKS_DATEFORMATMANUAL . '</td></tr>';
	$form .= '</table>';
	return $form;
}
